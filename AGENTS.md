# GodotAP: Godot 4.4 → 3.6 Downpatch Reference
...
Godot 3.6 scene loader/`Control::_set` (control.cpp:223-273) ignores the `theme_constants/<name>` property prefix → `get_constant()` returns 0. Use `custom_constants/<name>` instead, which populates `data.constant_override` and is read by `Control::get_constant()`.
...
### Typed Array Method Removal
| 4.x | 3.6 |
|---|---|
| `arr.assign(values)` | `arr.clear(); for v in values: arr.append(v)` |
| `arr.make_read_only()` | wrap in class with getter returning `arr.duplicate()` or make `const` if static; `make_read_only()` is not available |
| `arr.pop_at(i)` | `arr.remove(i)` |
| `dict.find_key(v)` | manual iteration: `for k in dict: if dict[k] == v: return k` |
| `str.validate_filename()` | manual sanitization needed |

### Packed Array Types
- `PackedByteArray` → `PoolByteArray`
- `PackedStringArray` → `PoolStringArray`
- `Array[int]` (typed) → `Array` (untyped)
- `Array[String]` → `Array`
...
Godot 3.6 has no `Texture2D` class (4.x rename); textures are plain `Texture`. A `.tres`/`.tscn` ext_resource header still saying `type="Texture2D"` fails to load: `ERROR: Cannot get class 'Texture2D'. (core/class_db.cpp:324)` per occurrence, and each referenced icon resolves to **null** (checkbox/radio/arrow icons silently invisible).
...
### Section 2: GDScript Syntax Rules (3.6)
...
|@export_range(0.0, 20.0, |0.5)  # 3.6 may fail with float args — use ints: export(int, 0, 20, 1)|
...
- `arr.filter(func(x): return ...)` ✓
- `arr.map(func(x): return ...)` ✓
- `arr.front()` ✓
- `arr.back()` ✓
- `arr.pop_front()` ✓
- `arr.pop_back()` ✓
- `arr.resize(n)` ✓
- `arr.size()` ✓
- `arr.is_empty()` — **NOT available**, use `arr.size() == 0`
- `arr.clear()` ✓
- `arr.reverse()` ✓
- `arr.assign(values)` — **NOT** available (4.0+), use `arr.clear(); for v in values: arr.append(v)`
- `arr.make_read_only()` — **NOT** available (4.0+), wrap in class with getter returning `arr.duplicate()` or make `const` if static

### Dictionary Operations (3.6)
- `dict.keys()` ✓
- `dict.values()` ✓
- `dict.has(key)` ✓
...
- `dict.duplicate()` ✓
- `dict.merge(other)` ✓
- `dict.find_key(val)` — **NOT** available (4.0+), use: `for k in dict: if dict[k] == val: return k`
...
| `godot_ap/ui/themes/symbols_font.tres`, `themes/basic_font.tres`, `ui/console_font.tres` | DynamicFont `fallback/N` |
